...
describe("updateAuthShareTokenByIdList module", () => {
  let sandbox;
  let updateAuthShareTokenByIdList;
  ...
  beforeEach(() => {
    ...
    updateAuthShareTokenByIdList = proxyquire(
      "../../../../../src/db-layer/main/AuthShareToken/utils/updateAuthShareTokenByIdList",
      {
  ...
  describe("updateAuthShareTokenByIdList", () => {
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateAuthShareTokenByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      AuthShareTokenStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateAuthShareTokenByIdList(["id1"], {
        active: false,
      });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateAuthShareTokenByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateAuthShareTokenByIdList(["id1"], {});
      sinon.assert.calledOnce(AuthShareTokenStub.update);
    });
  });
});
