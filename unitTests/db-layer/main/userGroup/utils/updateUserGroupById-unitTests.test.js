...
describe("updateUserGroupById module", () => {
  let sandbox;
  let updateUserGroupById;
  let UserGroupStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated UserGroup" };
  ...
  beforeEach(() => {
    ...
    updateUserGroupById = proxyquire(
      "../../../../../src/db-layer/main/UserGroup/utils/updateUserGroupById",
      {
  ...
  describe("updateUserGroupById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateUserGroupById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateUserGroupById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateUserGroupById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-only-input with ID and dataClause", async () => {
      const result = await updateUserGroupById({ id: fakeId, name: "Updated" });
      expect(result).to.deep.equal(fakeUpdatedData);
    });

    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateUserGroupById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateUserGroupById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      UserGroupStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateUserGroupById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
