...
describe("deleteUserGroupById module", () => {
  let sandbox;
  let deleteUserGroupById;
  let UserGroupStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted UserGroup" }),
    };
    ...
    deleteUserGroupById = proxyquire(
      "../../../../../src/db-layer/main/UserGroup/utils/deleteUserGroupById",
      {
  ...
  describe("deleteUserGroupById", () => {
    it("should delete UserGroup and index removal in Elastic", async () => {
      const result = await deleteUserGroupById(fakeId);

      expect(result).to.deep.equal({ id: fakeId, name: "Deleted UserGroup" });
    ...
    it("should support object as ID input", async () => {
      ...
      UserGroupStub.findOne.resolves(localMock);

      const result = await deleteUserGroupById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteUserGroupById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteUserGroupById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      UserGroupStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteUserGroupById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted UserGroup" }),
      };
      UserGroupStub.findOne.resolves(localMock);

      const result = await deleteUserGroupById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted UserGroup",
      });
      sinon.assert.calledOnce(localMock.update);
    });
  });
