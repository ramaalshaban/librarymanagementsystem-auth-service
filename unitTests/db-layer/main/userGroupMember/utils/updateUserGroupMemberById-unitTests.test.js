...
describe("updateUserGroupMemberById module", () => {
  let sandbox;
  let updateUserGroupMemberById;
  let UserGroupMemberStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated UserGroupMember" };
  ...
  beforeEach(() => {
    ...
    updateUserGroupMemberById = proxyquire(
      "../../../../../src/db-layer/main/UserGroupMember/utils/updateUserGroupMemberById",
      {
  ...
  describe("updateUserGroupMemberById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateUserGroupMemberById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateUserGroupMemberById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateUserGroupMemberById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateUserGroupMemberById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateUserGroupMemberById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      UserGroupMemberStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateUserGroupMemberById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
