...
describe("deleteUserGroupMemberById module", () => {
  let sandbox;
  let deleteUserGroupMemberById;
  let UserGroupMemberStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted UserGroupMember" }),
    };
    ...
    deleteUserGroupMemberById = proxyquire(
      "../../../../../src/db-layer/main/UserGroupMember/utils/deleteUserGroupMemberById",
      {
  ...
  describe("deleteUserGroupMemberById", () => {
    it("should delete UserGroupMember and index removal in Elastic", async () => {
      const result = await deleteUserGroupMemberById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted UserGroupMember",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      UserGroupMemberStub.findOne.resolves(localMock);

      const result = await deleteUserGroupMemberById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteUserGroupMemberById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteUserGroupMemberById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      UserGroupMemberStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteUserGroupMemberById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted UserGroupMember" }),
      };
      UserGroupMemberStub.findOne.resolves(localMock);

      const result = await deleteUserGroupMemberById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted UserGroupMember",
      });
