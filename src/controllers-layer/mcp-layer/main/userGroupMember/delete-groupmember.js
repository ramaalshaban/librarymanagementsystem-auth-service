...
class DeleteGroupMemberMcpController extends AuthMcpController {
  constructor(params) {
    super("deleteGroupMember", "deletegroupmember", params);
    this.dataName = "userGroupMember";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteGroupMemberManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description: "This route is used by admin to delete a member from a group.",
    parameters: DeleteGroupMemberMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteGroupMemberMcpController = new DeleteGroupMemberMcpController(
        mcpParams,
      );
      try {
        const result = await deleteGroupMemberMcpController.processRequest();
        //return DeleteGroupMemberMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
