...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("authShareToken");
  await elasticIndexer.indexData(data);
};

const updateAuthShareTokenById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AuthShareToken.findOne({
    ...
    const [rowsCount, [dbDoc]] = await AuthShareToken.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateAuthShareTokenById;
